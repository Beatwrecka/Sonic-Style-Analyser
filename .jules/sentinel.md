...
## 2026-06-12 - Enforce Service-Level Input Validation for External Links
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` accepted external URLs without independent validation. Even though the frontend has some validation, passing unvalidated URLs directly to the service layer exposes the application to Server-Side Request Forgery (SSRF) and Prompt Injection if UI-level protections are bypassed or if the API is called directly.
**Learning:** Relying solely on client-side UI validation is insufficient. Core services and backend-facing API wrappers must independently validate their inputs (Defense in Depth) to prevent unexpected behavior or exploitation.
**Prevention:** Always enforce independent input validation at the service layer (e.g., domain and protocol checks using utility functions like `validateAndNormalizeUrl` before incorporating user input into LLM prompts or API calls).
