...
## 2026-06-19 - Enforce Domain Allowlist and Safe URL parsing at the Service Layer
**Vulnerability:** Even though the UI (or `App.tsx`) was performing safe URL validation by calling `validateAndNormalizeUrl`, the underlying core service function `analyzeLink` in `services/geminiService.ts` would process any raw `url` provided if it was invoked directly. This exposed the API integration to SSRF and Prompt Injection risks via unvalidated URLs if the function was called from elsewhere or UI checks were bypassed.
**Learning:** Relying solely on caller/UI-layer checks violates the Defense in Depth principle. Core service functions must independently validate their inputs before passing them to external systems or LLMs.
**Prevention:** Apply `validateAndNormalizeUrl` directly within `analyzeLink` to guarantee URLs are always validated against the strict domain allowlist and protocol checks before hitting the LLM model.
