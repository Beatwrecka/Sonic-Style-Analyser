...
## 2026-06-16 - Defense in Depth: Service-Level URL Validation
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was relying entirely on UI-level checks in `App.tsx` to validate that the provided URL was safe (e.g., protocol and domain). If the API was called directly or if the UI validation was bypassed, it could attempt to process unsafe URLs, potentially leading to SSRF or prompt injection.
**Learning:** Depending solely on client-side UI validation is insufficient for security. Core services and backend-facing API wrappers must independently validate their inputs to ensure defense in depth. This prevents unexpected behavior or potential exploitation if the UI layer is circumvented.
**Prevention:** Always enforce independent input validation at the service layer (e.g., URL validation using `validateAndNormalizeUrl` before making external API calls), even if the UI also performs similar checks for user experience.
