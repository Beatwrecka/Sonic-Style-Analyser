...
## 2025-05-20 - Missing Defense in Depth for Input Validation
**Vulnerability:** The `geminiService.ts` core methods `analyzeAudioFile` and `analyzeLink` lacked independent input validation (MIME type checking and URL normalization/validation, respectively), relying entirely on the UI layer (e.g., `App.tsx`) to sanitize inputs. If another part of the application bypassed the UI layer or the API was called directly, malicious input could reach external systems or prompt construction logic.
**Learning:** Security controls should not be confined solely to the UI presentation layer. "Defense in Depth" mandates that core service/API interfaces strictly validate their inputs independently to prevent SSRF, malicious file processing, and prompt injection if UI checks are bypassed.
**Prevention:** Always implement self-contained validation logic within core service functions. For example, explicitly check that file MIME types begin with `audio/` and sanitize/allow-list URLs within the service module before they are passed into models or external queries.
