...
## 2026-06-05 - Defense in Depth: Service-Level URL Validation
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was passing the user-provided URL string directly to the Gemini API prompt without explicit validation. Although `App.tsx` validated URLs beforehand, calling the service function independently would bypass these checks, leading to a risk of Server-Side Request Forgery (SSRF) and Prompt Injection via malicious inputs.
**Learning:** Relying solely on client UI-level validation creates a false sense of security. Core services and backend-facing logic must independently validate their inputs, employing a Defense in Depth strategy to ensure safe handling of arbitrary input regardless of how the function is invoked.
**Prevention:** Always validate external, user-provided inputs (like URLs) at the service level before using them. In this case, `validateAndNormalizeUrl` was implemented in the `analyzeLink` function to proactively validate and sanitize the URL before it reaches the prompt context.
