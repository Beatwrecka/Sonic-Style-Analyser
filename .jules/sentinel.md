...
## 2025-05-21 - Service Layer Input Validation (Defense in Depth)
**Vulnerability:** The application was relying solely on frontend UI-level validation to restrict file uploads to `audio/` types. If a user bypassed the frontend (e.g., using API tools), malicious or unexpected file types could be sent to the backend `analyzeAudioFile` service, leading to potential unhandled errors or unsafe interactions with external APIs.
**Learning:** Security controls should be implemented as close to the data access layer or service boundary as possible. Relying exclusively on client-side or UI-level checks is insufficient because they can be easily bypassed.
**Prevention:** Implement explicit input validation (like MIME type checking) at the entry points of all service layer functions, regardless of any upstream validation. This ensures Defense in Depth and protects the service independently of how it is invoked.
