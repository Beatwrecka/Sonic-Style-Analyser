...
## 2026-05-29 - Defense in Depth: Service-Level URL Validation
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was directly using user-provided URLs in external API calls (Gemini Prompt). Although there was a UI-level validation in `App.tsx` (`validateAndNormalizeUrl(url)`), bypassing the UI and calling the service layer directly would result in potential SSRF or prompt injection.
**Learning:** Relying purely on UI validation leaves service functions vulnerable to unexpected arguments. Service functions should independently enforce security boundaries, particularly when passing data to external APIs, to follow Defense in Depth principles.
**Prevention:** Always implement service-level input validation independent of UI checks. Centralize core validations (like domain allowlists) into utility modules and call them directly within the service entrypoints before further processing.
