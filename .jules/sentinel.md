...
## 2025-06-07 - Defense in Depth: Service-Level External Link Validation
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was passing raw, unvalidated URLs into the Gemini AI prompt template. While the frontend UI (`App.tsx`) was running a validation check (`validateAndNormalizeUrl(url)`), the `geminiService.ts` function lacked independent service-level validation. This creates a risk where a malicious user could bypass the UI validation and send dangerous URLs directly to the service layer, leading to Server-Side Request Forgery (SSRF) or Prompt Injection attacks.
**Learning:** Depending entirely on client-side UI validation is an insecure practice known as "Security Theater." Defense in Depth demands that core logic and service functions validate their own inputs independently, assuming that upstream UI-level checks might be bypassed.
**Prevention:** Always enforce independent input validation within the service layer (e.g., using `validateAndNormalizeUrl`) before processing external URLs or user inputs, regardless of any prior checks performed in the UI components.
