...
## 2025-06-13 - Defense in Depth: Service-Level URL Validation
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was passing raw, user-provided URLs directly to the Gemini LLM prompt. Even if the UI validated the URL beforehand, the service itself lacked validation. If the service were invoked programmatically or if UI checks were bypassed, it could allow an attacker to inject arbitrary strings or unauthorized domains, leading to Server-Side Request Forgery (SSRF) or Prompt Injection.
**Learning:** Depending solely on client-side UI validation is insufficient for core services. Backend-facing API wrappers must independently validate their inputs (like URLs) to ensure defense in depth, especially when inputs are directly embedded in LLM prompts.
**Prevention:** Always enforce independent input validation at the service layer. Ensure user-provided URLs are parsed, validated against allowed protocols/domains, and normalized before being incorporated into prompts or external API calls.
