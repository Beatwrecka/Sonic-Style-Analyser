...
## 2025-05-29 - Missing Service-Level Validation in analyzeLink
**Vulnerability:** The `analyzeLink` function in `services/geminiService.ts` was not validating the URL at the service layer. While URL validation existed, it was not being utilized correctly in the core function. This could potentially expose the application to Server-Side Request Forgery (SSRF) and Prompt Injection if an attacker bypassed the UI and called the service method directly with a malicious URL.
**Learning:** Depending solely on upstream or UI-level validation leaves backend services vulnerable. Every service function must enforce its own validation independently as a critical part of defense in depth.
**Prevention:** Always implement service-level input validation independently of the UI layer. Ensure external inputs, such as URLs, are verified against a strict allowlist (e.g., using `validateAndNormalizeUrl`) before being passed to external API models or execution paths.
