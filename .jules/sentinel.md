...
## 2026-05-18 - Prompt Injection and SSRF via Unvalidated URL Domain
**Vulnerability:** The application was vulnerable to Prompt Injection and Server-Side Request Forgery (SSRF) because it only checked the protocol of a user-provided URL but did not restrict the domain. A malicious user could provide a URL pointing to a malicious site or an internal resource, which the application would then fetch and use as context for the Gemini model analysis. This could lead to the model returning attacker-controlled output or the application exposing internal resources.
**Learning:** Checking only the protocol of a URL is insufficient for security when the URL is used to fetch remote content or context. Attackers can easily bypass protocol checks by hosting malicious content on an allowed protocol (e.g., `http://malicious-site.com`).
**Prevention:** Always implement a strict domain allowlist when accepting user-provided URLs that are used to fetch content or as context for external services (like LLMs). Ensure that the domain validation logic correctly parses the URL and accounts for subdomains or path trickery.
