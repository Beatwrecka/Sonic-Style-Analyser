...
import { validateAndNormalizeUrl, ERROR_INVALID_URL, ERROR_INVALID_PROTOCOL, ERROR_UNALLOWED_DOMAIN } from './services/urlUtils';
...
const App: React.FC = () => {
  ...
  const handleAnalyzeUrl = async (url: string) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (
          error.message === ERROR_INVALID_URL ||
          error.message === ERROR_INVALID_PROTOCOL ||
          error.message === ERROR_UNALLOWED_DOMAIN
        ) {
          showAlert('error', error.message);
        } else {
